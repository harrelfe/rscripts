...
f <- ols(log(energy) ~ log(mass) * caste, data=d)
f
latex(f)
g <- ols(log(energy) ~ log(mass) + caste, data=d)
g
latex(g)
```
...
We use studentized residuals, which for now need to be computed using the build-in `lm` function. [See [this](https://stackoverflow.com/questions/4357031) for a discussion of using `ggplot2` for qqnorm plots.]{.aside}

```{r}
flm <- lm(log(energy) ~ log(mass) * caste, data=d)
glm <- lm(log(energy) ~ log(mass) + caste, data=d)
rf  <- rstandard(flm)
rg  <- rstandard(glm)

ggplot(mapping=aes(fitted(flm), rf)) + geom_point()
ggplot(mapping=aes(sample=rf)) + geom_qq() + stat_qq_line() +
  xlab('Studentized Residuals') + ylab('Normal Quantiles')
ggplot(mapping=aes(fitted(glm), rg)) + geom_point()
