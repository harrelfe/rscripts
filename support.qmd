...
* Predict `log(slos)`
* This predicts mean `slos` which under a normality assumption for the residuals is also the median `slos`
...
ols(log(slos) ~ pol(num.co, 2), data=d)
ols(log(slos) ~ pol(pmin(num.co, 6), 2), data=d)
ols(log(slos) ~ num.co7, data=d)
