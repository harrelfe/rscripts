...
```{r}
#| fig.height: 4
getRs('princmp.r')   # gives a lot of useful output and decodes PCs with stepwise regression
pc <- princmp(~ age + ht + wt + leg + arml + armc + waist + tri + sub +re,
              data=d, sw=TRUE)
...
```{r}
spc <- princmp(~ age + ht + wt + leg + arml + armc + waist + tri + sub +re,
               data=d, sw=TRUE, method='sparse')
```

* Let's see how regular and sparce PCs predict glycohemoglobin in an ordinal model

```{r}
for(k in 1:5) print(orm(gh ~  pc[, 1:k], data=d))
for(k in 1:5) print(orm(gh ~ spc[, 1:k], data=d))
```
