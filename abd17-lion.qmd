...
# Must put results='asis' in chunk headers that use this html output
getRs('abd.r')
```
...
A scatterplot is an excellent way to present the bivariate relationship when there are not many ties on x-y combinations, which would create coincident points that would not be visible on the plot.  Create a function that counts the number of coincident points to within the resolution we expect the plot symbols to have (e.g., 200 points in both x- and y-directions).  We'll add the number of coincident points as a subtitle on the graph.
...
ncoinc <- function(x, y) {
  ...
  rx <- range(x)
  ry <- range(y)
  x  <- round((x - rx[1]) / diff(rx) * 200)
  y  <- round((y - ry[1]) / diff(ry) * 200)
  z  <- paste(x, y)
  length(z) - length(unique(z))
}
# with(d, ...) evaluates ... using variables inside d
nc <- with(d, ncoinc(prop, age))
```
...
Write a function that will retrieve labels and units stored in our dataset's metadata and format them into an expression that allows `ggplot2` to change fonts for units.  
[This will have higher payoff when dealing with dozens of variables. See [this](http://hbiostat.org/rflow/graphics.html#ggplot2) for more about plotting labels.]{.aside}
...
ggplot(Predict(f, prop=pseq)) +
  geom_point(aes(prop, age), d)
```
...
r     <- resid(f)
rstud <-  rstudent(lm(age ~ prop, data=d))
ggplot(mapping=aes(r, rstud)) + geom_point()
```

For this one-predictor case the two types of residuals are telling us the same thing.  So just plot the ordinary residual vs. the predictor.
...
# Combine two types of estimates (point estimates don't vary)
u <- rbind(Mean=a, Individual=b)
ggplot(u) +
...
Compute the mean absolute prediction error (mean $|\hat{\epsilon}|$).  This has the advantages of
...
$\rightarrow$ predicted ages based on proportion black nose are off by `r round(mean(abs(r)), 2)` years on the average.  This can be compared against the typical difference in ages between lions in the sample (Gini's mean difference) of only`r round(GiniMd(d$age), 1)`.  This is in line with the estimated $R^{2}_\text{adj}$ of only 0.61.  The model is good at predicting long-term average but not so much for estimating ages of individual lions.
