...
bmovie <- function(x) {
  ...
  linkit('images/movie.png', file)
  }

movie <- function(x) linkit('images/movie.png', x)

ddisc <- function(x) {
  file <- paste0('http://bit.ly/datamethods-', qproject, x)
  linkit('images/discourse.png', file)
  }
...
disccom <- function(topic)
  if(! ishtml) invisible() else paste0('[[![](images/discourse.png)](https://discourse.datamethods.org/t/', topic, ')<br>',
